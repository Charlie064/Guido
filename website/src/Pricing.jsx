...
import { FLASH_PINK, Logo } from "./brand.jsx";
...
function PricingCard({ tier, currencySymbol }) {
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        onClick={handleClick}
        className="mt-8 inline-flex items-center justify-center rounded-full font-semibold px-6 py-3 text-sm text-white transition-transform duration-150 hover:scale-[1.02]"
        style={{
...
export default function Pricing() {
  ...
  return (
    <div
      className="min-h-screen text-[#0A0A0A] bg-white"
    ...
    >
      <header className="sticky top-0 z-30 backdrop-blur-md bg-white/85 border-b border-black/[0.06]">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <a href="/" className="text-sm font-medium text-neutral-600 hover:text-black transition-colors">
            Back to site
          </a>
        </div>
      </header>
      ...
      <div className="pt-20 pb-24 px-6">
        ...
        <div className="max-w-2xl mx-auto flex flex-wrap justify-center items-stretch gap-6">
          {TIERS.map((tier) => (
            <PricingCard key={tier.id} tier={tier} currencySymbol={currencySymbol} />
          ))}
        </div>
