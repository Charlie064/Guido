import { type Env, json, membershipFromBearer, monthlyVoiceSecondsUsed } from "./auth";
...
// git history). $2/month flat, shared by every plan: this is a
// cost-control measure against runaway usage, not a metered product tier
// like skill_runs — see monthlyVoiceSecondsUsed's comment.
...
export async function handleVoiceTranscribe(request: Request, env: Env, auth: ReturnType<typeof createAuth>): Promise<Response> {
  ...
  const used = await monthlyVoiceSecondsUsed(env.DB, member.user_id);
  if (used >= MONTHLY_CAP_SECONDS) {
    return json({ error: `Monthly voice quota used up ($${MONTHLY_CAP_USD} cap) — resets next month, or type your goal instead.` }, 403);
  }
