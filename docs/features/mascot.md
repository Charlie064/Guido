...
- **What’s wired:** website can import the React components via the
  ...
  buddy on pointer devices. Desktop login shows the idle SVG. **Not
  yet:** desktop overlay that follows the OS cursor, or driving
  `thinking` / `success` from live locate/research calls.
...
- **Plain `<img>` (desktop sidebar, emails, favicon):**
  ...
  `assets/mascot/sync-static.sh` after regenerating.
