...
def research_goal(client: anthropic.Anthropic, goal: str) -> list[dict]:
    prompt = (
        f'A user wants to do this in some application: "{goal}". '
    ...
    response = client.messages.create(
        ...
        # search before it has to just answer.
        max_tokens=4096,
        tools=[{"type": "web_search_20260209", "name": "web_search", "max_uses": 3}],
        messages=[{"role": "user", "content": prompt}],
    )

    # Sometimes a trailing text block is blank (observed live) — take the
    # last non-blank one, not just the last one.
...
def main() -> None:
    ...
    if len(sys.argv) != 2:
        print(f'Usage: python {sys.argv[0]} "<goal>"', file=sys.stderr)
        sys.exit(1)
    ...
    try:
        steps = research_goal(client, goal)
